...
simset <- expand.grid(genome_len = genome_len , mut_rate = mut_rate, pop.init = pop.init, samp = c(15, 20), dec.rate = dec.rate);

simset <- simset %>%
    mutate(filename =  paste0("c:/temp/slim_", pop.init, "_", dec.rate, "_", samp, filename_lab));

decline_set <- slim_script_render(decline_script, template = simset);

slim_decline <- function(genome_len, mut_rate, pop.init, dec.rate, filename_lab) {
  
  #create combinations of all variables
  simset <- expand.grid(genome_len = genome_len , mut_rate = mut_rate, pop.init = pop.init, dec.rate = dec.rate);
  
  #create filenames for variable combinations
  simset <- simset %>%
    mutate(filename =  paste0("c:/temp/slim_", pop.init, "_", dec.rate, "_", pop.sample, filename_lab));
  ...
  slim_block(2050,late(),
             {
               ...
               slimr_output(p1.outputVCFSample(sampleSize=slimr_template("samp"), replace=F,  outputMultiallelics=F, filePath=nn,  simplifyNucleotides=T), name = "nn");
