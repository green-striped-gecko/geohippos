...
gls[[1]] <- geohippos::gl.read.vcf("./vcf/slim_200-50-100y_p1.vcf", verbose=0)
gls[[2]] <- geohippos::gl.read.vcf("./vcf/slim_200-50-100y_p2.vcf")
gls[[3]]<- geohippos::gl.read.vcf("./vcf/slim_200-50-100y_p3.vcf")
gls[[4]] <- geohippos::gl.read.vcf("./vcf/slim_200-50-100y_p4.vcf")
gls[[5]]<- geohippos::gl.read.vcf("./vcf/slim_200-50-100y_p5.vcf")
#gls <- geohippos::gl.read.vcf("d:/downloads/slim_50_100inlast10.vcf")
#split chromosomes...

sf <-lapply(gls, gl.sfs)
msf <- do.call(rbind,sf)
yy<- msf[c(1,5),]
sssh <- apply(yy,2, function(x)  sum(yy)/sum(rowSums(yy)/x))

croc <- readRDS("./vcf/croc.rda")

frogs <- readRD

glsb <- do.call(cbind, gls[1:4])
...
#L <- 2.78e9 #length of croc genome
mu <- 1e-8  #mutation rate
###############
# Neestimator #
###############
system.time(
  Ne_ldnest <- gl.LDNe(gls[[1]],neest.path = paste0("./binaries/NEestimator/",os), singleton.rm = F, critical = c(0))
)
...
###############
system.time(
  Ne_epos <- gl.epos(gls, epos.path = paste0("./binaries/epos/",os), l = 1e9, u=mu, boot=100)
)

Ne_epos <- gl.epos(gls, sfs =sssh[-1] , epos.path = paste0("./binaries/epos/",os), l = L, u=mu, boot=200)
...
dummy$year <- c(0,25,   25, 50 , 50, 500)
dummy$Ne <- c(100,100,20, 20, 500 ,500)
